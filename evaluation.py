...
def mlp_evaluator(x, y, learning_rate=1e-3):
    ...
    for train_index, test_index in skf.split(x, y):
        ...
        data_loader = DataLoader(dataloader.MLPDataset(x[train_index], y[train_index]), batch_size=128, shuffle=True)
        data_loader_eval = DataLoader(dataloader.MLPDataset(x[test_index], y[test_index]), batch_size=128, shuffle=True)
